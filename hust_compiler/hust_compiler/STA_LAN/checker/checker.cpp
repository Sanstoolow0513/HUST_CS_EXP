...
void Checker::visit(ReturnStmtAST &ast) {
  ...
  auto entry = this->find_func();
  // 函数返回类型不匹配
  if (entry->type != this->current_type.type) {
    // 使用当前函数名作为错误位置
    err.error(ErrorType::FuncReturnTypeNotMatch, current_func_name);
...
void Checker::visit(IterationStmtAST &ast) {
  ...
  if (ast.stmt) {
    bool prev_in_loop = this->in_loop;
    this->in_loop = true;
    ast.stmt->accept(*this);
...
void Checker::visit(CallAST &ast) {
  ...
  // 在符号表中查找对应函数
  Entry *entry = this->Lookup(*ast.id);
...
void Checker::visit(RelExpAST &ast) {
  ...
  this->Expr_int = true; // 关系表达式结果为整型(0或1)
  this->current_type.type = TYPE::TYPE_INT;
}
...
void Checker::visit(EqExpAST &ast) {
  ...
  this->Expr_int = true; // 等式表达式结果为整型(0或1)
  this->current_type.type = TYPE::TYPE_INT;
}
...
void Checker::visit(LAndExpAST &ast) {
  ...
  this->Expr_int = true; // 逻辑表达式结果为整型(0或1)
  this->current_type.type = TYPE::TYPE_INT;
}
