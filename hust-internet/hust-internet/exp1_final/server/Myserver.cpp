...
void sendResponse(SOCKET sessionSocket, std::string& filePath, const std::string& root_dir) {
	...
	if (!file)
	{
		std::string default404Path = root_dir + "/404.html";
		std::cout << "Trying to open 404.html at path: " << default404Path << std::endl;
		std::ifstream default404File(default404Path, std::ios::binary);
		response = "HTTP/1.1 404 Not Found\r\n";
		contentType = "text/html";
		file = std::move(default404File);
	}
	...
	std::cout << "Sent headers to client: " << response << std::endl;
...
void handleRequest(SOCKET sessionSocket, const char* requestData, int dataLength, string root_dir) {
	...
	sendResponse(sessionSocket, filePath, root_dir);
...
void main() {
	...
	while (true) {
		...
		FD_ZERO(&wfds);
		FD_SET(srvSocket, &rfds);
		if (!first_connetion&&sessionSocket != INVALID_SOCKET)
		{
		...
		if (sessionSocket != INVALID_SOCKET) {
			if (FD_ISSET(sessionSocket, &rfds)) {
				...
				if (rtn > 0) {
					...
					std::cout << "Received " << rtn << " bytes from client." << std::endl;
					std::cout << "Raw request: " << std::string(recvBuf, rtn) << std::endl;
